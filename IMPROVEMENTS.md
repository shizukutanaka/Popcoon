...
- **問**: ホーム画面ウィジェットは各ウォッチ項目に「買い時 (BUY_NOW/WAIT)」バッジを
  `WidgetVerdict` で表示している。では、ユーザーが項目を**管理する**アプリ内の
  ウォッチリスト画面は、買い時を表示しているか?
- **答**: していない。ウォッチリスト行は価格・追加時変動・目標/在庫チップだけで、
  **買い時判定が無い**。
- **帰結**: ウィジェットは「今が買い」と知らせるのに、それを見てアプリを開くと
  ウォッチリストは価格しか出さない。**最も行動 (購入) に近い画面**で、価格追跡アプリの
  核心価値「いつ買うべきか」を取りこぼしている。ウィジェットとアプリで判定が分裂している。

### 設計判断

- `WidgetVerdict.forItem(realPrice, targetPrice, addedPrice)` は **WatchlistItem の
  フィールドだけで動く純関数** (履歴取得・ネットワーク不要、`WidgetVerdictTest` で検証済み)。
  → ウォッチリスト画面で**そのまま再利用**でき、N 件ぶんの backend 往復も発生しない。
- 表示は既存の `VerdictBadge` (検索結果と同じ・4 ロケール文字列既存) を再利用し、
  ウィジェット/検索/ウォッチリストで**買い時表現を統一**。
- **NEUTRAL はバッジ非表示** (null)。全行にバッジを出すと視覚ノイズになるため、
  BUY_NOW / WAIT の actionable な行だけ強調する (詳細画面の警告/エコ表示と同じ方針)。

### 適用した変更

1. **`watchlistBuyVerdict(item): BuyTimingScorer.Verdict?`** を新設 (テスト可能な純関数)。
   `WidgetVerdict` の String 判定を enum にマップし、NEUTRAL は null。
2. **`WatchlistRow`** の価格行に、`watchlistBuyVerdict` が非 null のとき `VerdictBadge` を表示。
   行は merge 済み (Tier 61) なので TalkBack は「買い時, [タイトル], ¥価格」と一括読み上げ。
3. **`WatchlistBuyVerdictTest`** 5 ケース: 目標到達/有意下落=BUY_NOW、有意上昇=WAIT、
   横ばい・基準なし・価格不明=null (NEUTRAL 非表示の製品判断を固定)。

### 一般教訓

「同じ判定ロジックを別 UI で見せているか?」は機能の一貫性を測る良い問い。
**ウィジェットという周辺面にあって本体画面に無い情報**は、たいてい本体にあるべき
(ユーザーは周辺で気づき、本体で行動する)。純関数で実装された判定は UI 横断で
再利用でき、再計算もネットワークも増やさず一貫性を担保できる — ロジックを
Android 非依存の純関数に保ってきた設計 (Tier 多数) がここで効いた。
