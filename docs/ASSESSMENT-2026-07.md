...
6. **i18n 規律** — 4 ロケール × 365 キー完全一致 (2026-08 に `check_i18n.py` で
   **キー集合・位置指定子・原文混入**まで検査するようにした。従来はキー数のみで、
   ja に A / en に B があっても通っていた。実測の結果、欠陥は 0 件で主張は裏付けられた) + kind/label 分離パターンでオラクル結合と
...
2. **Compose/Room/Hilt 層はコンパイル未検証** — 環境に Android SDK が無い。純 Kotlin は parity で
   ...
   **2026-08 に現実の欠陥として顕在化**: `UserPreferences` の 5 メンバーが `override` 欠落で
   ...
   2026-08 に静的ゲートを 9 種へ拡張 (override / `R.*` 参照 / `when` 網羅 / テスト参照 /
   `realPrice` の ¥0 除外 / Room 移行チェーン / `take` の前の順序付け / ビルド構成の整合 / i18n の整合) し、
