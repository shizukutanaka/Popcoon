...
7. **backend の実ランタイムテスト** — vitest-pool-workers で本物の `src/index.ts` を miniflare 上で
   実行 (65 tests)。レート制限はネイティブ binding + KV フォールバックの漸進移行、
...
| C4 | backend テスト追加 | 未カバー経路 (異常系ヘッダ・CORS・巨大 payload 境界値) を worker.test.ts に | vitest 65+ / tsc |
...
- backend: **tsc 0 errors / vitest 65 tests pass**
