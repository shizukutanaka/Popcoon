...
## 17. 「4 ロケール完全一致」を主張に見合う検査にした (2026-08、静的ゲート 9 種目)

ASSESSMENT は長所 #6 に「i18n 規律 — 4 ロケール × 365 キー完全一致」を掲げているが、
実際のゲート (`ci/verify.py`) は **キーの個数**しか見ていなかった。
個数が同じでも ja に A / en に B があれば通るし、書式指定子がずれれば
`getString(id, args)` が **実行時に例外**を投げる。主張と検査の強度が食い違っていた
(§7 の「宣言と実装の食い違い」がゲート側に出た形)。

### 測ってみたら i18n 自体はクリーンだった

`check_i18n.py` の 5 検査を全ロケールに当てた結果、**実際の欠陥は 0 件**。
未翻訳の残留も、キー集合のズレも、位置指定子の不一致も無い。
長所 #6 の主張は実測で裏付けられた — ただし今までそれを確かめる仕組みが無かった。

### ここでも自ツールが 2 回嘘をついた (4 回目・5 回目)

| 誤報 | 原因 |
|---|---|
| zh-rCN に「未翻訳の疑い 13 件」(重要/保存/警告/高/中/低 …) | **中国語と日本語は漢字を共有する**。漢字の有無で原文混入を判定してはいけない。かな (`ぁ-ゖァ-ヺ`) は中/韓/英に現れないので、そちらで判定する |
| 書式指定子の不一致 12 件 | `%%` (エスケープ済みリテラル) の 2 文字目を拾っていた。さらに `% e`(「+4% extra」) のような**書式適用されない文字列の裸の `%`** まで指定子と誤認。**位置指定子 (`%N$x`) だけ**を見れば誤検出ゼロになる |

裸の `%` は `sale_yahoo_5_day_desc` 等で正当に使われている (`stringResource(id)` のみで
書式適用されないため安全)。ここを緩めずに厳格化すると誤検出が出て、ゲート自体が信用されなくなる。

### 検査内容

(a) キー**集合**の一致 (個数一致より強い) / (b) 位置指定子の一致 /
(c) 位置指定子を持つ文字列に未エスケープの `%` が無いこと (確実にクラッシュする方) /
(d) かなの残留 = 未翻訳 / (e) 英語ロケールの CJK 残留。

欠陥注入 4 種 (キー削除 / 位置指定子を削る / 裸の `%` を入れる / かな混入) で全経路を実証。
