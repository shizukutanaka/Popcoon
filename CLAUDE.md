...
| app の kotest spec | `bash popcoon-tdd/kotlin_parity/run_kotest.sh` | **31 specs / 409 passed / 0 failed** (kotest jar 不在のためシム経由。63 中 32 は Android/Hilt/ktor 依存で対象外) |
...
- **kotest は jar 不在だが 31 spec は実行できる**: Maven Central への egress が無く kotest の jar を取得できないが、
  ...
  **テストファイルを 1 行も変えずに**コンパイル・実行する (409 アサーション)。残り 32 spec は production 側が
