...
|---|---|---|
| Python オラクル全体 | `cd popcoon-tdd && python3 -m pytest -q` | **476 passed, 1 skipped** |
| Kotlin 実行 parity 全体 | `bash popcoon-tdd/kotlin_parity/run_all.sh` | 全 13 ハーネス pass (`run.sh` は 136 matched / 0 mismatched) |
...
| i18n キー数一致 | `for f in values values-en values-ko values-zh-rCN; do grep -c '<string name=' app/src/main/res/$f/strings.xml; done` | **全ロケール 399** (plurals 3 は別) |
