#!/usr/bin/env bash
# run_rakuten.sh — execute-verify the pure RakutenMapper (DTO -> Product) WITHOUT an
# Android SDK, using the Kotlin compiler bundled in the Gradle distribution.
#
# RakutenClient itself depends on ktor + generated BuildConfig and cannot compile
# standalone; the conversion logic was extracted into RakutenMapper.kt (ktor/
...
java -cp "$LIB/*" org.jetbrains.kotlin.cli.jvm.K2JVMCompiler \
  -cp "$ST:$SER" -d "$OUT/rak.jar" -nowarn -no-reflect \
  "$SRC/data/model/Product.kt" \
  "$SRC/data/network/RakutenMapper.kt" \
  "$HERE/rakuten/RakutenMapperCheck.kt" 2>&1 | grep -v 'unable to find kotlin' || true

java -Dstdout.encoding=UTF-8 -cp "$OUT/rak.jar:$ST:$SER" \
  io.github.shizukutanaka.popcoon.data.network.RakutenMapperCheckKt
